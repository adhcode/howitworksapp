...
let SchedulerService = class SchedulerService {
   ...
   */ async sendPaymentReminderNotification(contract, type) {
        try {
            const dueDate = new Date(contract.nextPaymentDue);
            const today = new Date();
   ...
   */ async sendExpoNotification(tenantId, title, message) {
   ...
   */ async sendEmailNotification(tenantId, subject, message) {
        try {
   ...
   */ async processEscrowReleases() {
        ...
        let errorCount = 0;
        try {
            const results = await this.executeWithRetry(()=>this.landlordPayoutService.checkEscrowReleases(), 'escrow release check');
   ...
   */ async updateOverduePaymentStatuses() {
        ...
        try {
            ...
            const overdueContracts = await this.executeWithRetry(()=>this.db.select().from(_schema.tenantRentContracts).where((0, _drizzleorm.and)((0, _drizzleorm.lte)(_schema.tenantRentContracts.nextPaymentDue, today), (0, _drizzleorm.eq)(_schema.tenantRentContracts.status, 'active'))), 'overdue contracts query');
   ...
   */ async cleanupNotifications() {
        ...
        try {
            ...
            const sentResult = await this.executeWithRetry(()=>this.db.delete(_schema.paymentNotifications).where((0, _drizzleorm.and)((0, _drizzleorm.eq)(_schema.paymentNotifications.status, 'sent'), (0, _drizzleorm.lte)(_schema.paymentNotifications.sentAt, cutoffDate))), 'cleanup sent notifications');
            ...
            const failedResult = await this.executeWithRetry(()=>this.db.delete(_schema.paymentNotifications).where((0, _drizzleorm.and)((0, _drizzleorm.eq)(_schema.paymentNotifications.status, 'failed'), (0, _drizzleorm.lte)(_schema.paymentNotifications.createdAt, failedCutoffDate))), 'cleanup failed notifications');
            ...
            const staleResult = await this.executeWithRetry(()=>this.db.update(_schema.paymentNotifications).set({
                    status: 'failed',
                    sentAt: new Date()
                }).where((0, _drizzleorm.and)((0, _drizzleorm.eq)(_schema.paymentNotifications.status, 'pending'), (0, _drizzleorm.lte)(_schema.paymentNotifications.scheduledFor, staleCutoffDate))), 'update stale notifications');
            cleanedCount = (sentResult?.rowCount || 0) + (failedResult?.rowCount || 0) + (staleResult?.rowCount || 0);
            const duration = Date.now() - startTime;
            ...
            this.logOperationMetrics('notification_cleanup', {
                ...
                success: true,
                details: {
                    sentCleaned: sentResult?.rowCount || 0,
                    failedCleaned: failedResult?.rowCount || 0,
                    staleUpdated: staleResult?.rowCount || 0
                }
            });
        } catch (error) {
            const duration = Date.now() - startTime;
   ...
   */ logOperationMetrics(operation, metrics) {
   ...
   */ async getSchedulerHealth() {
        try {
            ...
            const [contractStats] = await this.db.select({
                total: (0, _drizzleorm.sql)`COUNT(*)`,
                active: (0, _drizzleorm.sql)`COUNT(*) FILTER (WHERE status = 'active')`
            }).from(_schema.tenantRentContracts);
            const today = new Date();
            today.setHours(0, 0, 0, 0);
            const [overdueStats] = await this.db.select({
                overdue: (0, _drizzleorm.sql)`COUNT(*) FILTER (WHERE next_payment_due < ${today} AND status = 'active')`
            }).from(_schema.tenantRentContracts);
            const [notificationStats] = await this.db.select({
                pending: (0, _drizzleorm.sql)`COUNT(*) FILTER (WHERE status = 'pending')`
            }).from(_schema.paymentNotifications);
            return {
                status: 'healthy',
                lastOperations: {
                },
    ...
    constructor(tenantPaymentService, landlordPayoutService, emailService, db){
        this.tenantPaymentService = tenantPaymentService;
        this.landlordPayoutService = landlordPayoutService;
        this.emailService = emailService;
        this.db = db;
        this.logger = new _common.Logger(SchedulerService.name);
        this.maxRetries = 3;
...
], SchedulerService.prototype, "checkDuePayments", null);
_ts_decorate([
...
_ts_decorate([
    (0, _schedule.Cron)('0 11 * * *'),
    _ts_metadata("design:type", Function),
    _ts_metadata("design:paramtypes", []),
    _ts_metadata("design:returntype", Promise)
], SchedulerService.prototype, "processEscrowReleases", null);
_ts_decorate([
    (0, _schedule.Cron)('0 12 * * *'),
    _ts_metadata("design:type", Function),
    _ts_metadata("design:paramtypes", []),
    _ts_metadata("design:returntype", Promise)
], SchedulerService.prototype, "updateOverduePaymentStatuses", null);
_ts_decorate([
    (0, _schedule.Cron)('0 2 * * *'),
    _ts_metadata("design:type", Function),
    _ts_metadata("design:paramtypes", []),
    _ts_metadata("design:returntype", Promise)
], SchedulerService.prototype, "cleanupNotifications", null);
SchedulerService = _ts_decorate([
    (0, _common.Injectable)(),
    _ts_param(3, (0, _common.Inject)(_databasemodule.DATABASE_CONNECTION)),
    _ts_metadata("design:type", Function),
    _ts_metadata("design:paramtypes", [
