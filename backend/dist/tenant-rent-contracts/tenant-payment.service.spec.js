...
describe('TenantPaymentService', ()=>{
    ...
    beforeEach(async ()=>{
        ...
        service = module.get(_tenantpaymentservice.TenantPaymentService);
        // Reset all mocks
        jest.clearAllMocks();
    });
    describe('calculateTransitionStartDate', ()=>{
        it('should calculate 3 months before expiry for monthly payout', ()=>{
            ...
            const result = service.calculateTransitionStartDate(expiryDate, _tenantrentcontractdto.LandlordPayoutType.MONTHLY);
            const expected = new Date('2024-09-01T00:00:00.000Z');
            expect(result).toEqual(expected);
        });
        it('should calculate 6 months before expiry for yearly payout', ()=>{
            const expiryDate = new Date('2024-12-01T00:00:00.000Z');
            const result = service.calculateTransitionStartDate(expiryDate, _tenantrentcontractdto.LandlordPayoutType.YEARLY);
        ...
        it('should return today if calculated date is in the past', ()=>{
            ...
            const result = service.calculateTransitionStartDate(pastExpiryDate, _tenantrentcontractdto.LandlordPayoutType.MONTHLY);
            expect(result.getTime()).toBeGreaterThanOrEqual(today.getTime());
        });
        it('should throw error for invalid payout type', ()=>{
            const expiryDate = new Date('2024-12-01T00:00:00.000Z');
            expect(()=>{
                service.calculateTransitionStartDate(expiryDate, 'invalid');
    ...
    describe('createRentContract', ()=>{
        ...
        it('should throw error if expiry date is in the past', async ()=>{
            ...
            await expect(service.createRentContract(pastExpiryDto)).rejects.toThrow(_common.BadRequestException);
        });
        it('should throw error if monthly amount is zero or negative', async ()=>{
            ...
            await expect(service.createRentContract(invalidAmountDto)).rejects.toThrow(_common.BadRequestException);
            const negativeAmountDto = {
                ...validContractDto,
                monthlyAmount: -1000
            };
            await expect(service.createRentContract(negativeAmountDto)).rejects.toThrow(_common.BadRequestException);
        });
        it('should throw error if tenant already has active contract', async ()=>{
            ...
            await expect(service.createRentContract(validContractDto)).rejects.toThrow(_common.BadRequestException);
        });
        it('should require original expiry date for existing tenants', async ()=>{
            ...
            await expect(service.createRentContract(existingTenantDto)).rejects.toThrow(_common.BadRequestException);
        });
        it('should validate original expiry date for existing tenants', async ()=>{
            const existingTenantDto = {
                ...validContractDto,
                isExistingTenant: true,
                originalExpiryDate: '2020-01-01T00:00:00.000Z'
            };
            await expect(service.createRentContract(existingTenantDto)).rejects.toThrow(_common.BadRequestException);
        });
    ...
    describe('getContractById', ()=>{
        ...
        it('should throw NotFoundException when contract not found', async ()=>{
            ...
            mockDb.where.mockReturnValue(mockDb);
            mockDb.limit.mockResolvedValue([]);
            await expect(service.getContractById('nonexistent')).rejects.toThrow(_common.NotFoundException);
        });
    });
    ...
    describe('updateNextPaymentDue', ()=>{
        ...
        it('should throw error if contract not found', async ()=>{
            ...
            mockDb.where.mockReturnValue(mockDb);
            mockDb.limit.mockResolvedValue([]);
            await expect(service.updateNextPaymentDue('nonexistent')).rejects.toThrow(_common.NotFoundException);
        });
    });
    describe('getContractsWithDuePayments', ()=>{
        it('should return contracts with payments due', async ()=>{
            ...
            const mockContracts = [
                {
                    id: 'contract-1',
                    nextPaymentDue: new Date('2024-01-10T00:00:00.000Z'),
                    status: 'active'
                },
                {
                    id: 'contract-2',
                    nextPaymentDue: new Date('2024-01-20T00:00:00.000Z'),
                    status: 'active'
                }
            ];
            mockDb.select.mockReturnValue(mockDb);
    ...
    describe('updateContractStatus', ()=>{
        it('should update contract status successfully', async ()=>{
            ...
            const result = await service.updateContractStatus('contract-123', _tenantrentcontractdto.ContractStatus.TERMINATED);
    ...
    describe('getExpiringContracts', ()=>{
        it('should return contracts expiring within specified days', async ()=>{
            const mockContracts = [
                {
                    id: 'contract-1',
                    expiryDate: new Date('2024-01-20T00:00:00.000Z'),
                    landlordPayoutType: 'yearly',
                    status: 'active'
                },
                {
                    id: 'contract-2',
                    expiryDate: new Date('2024-02-20T00:00:00.000Z'),
                    landlordPayoutType: 'yearly',
                    status: 'active'
                }
            ];
    ...
    describe('processMonthlyPayment', ()=>{
        ...
        it('should process payment successfully for monthly payout', async ()=>{
            ...
            expect(result.payoutType).toBe(_tenantrentcontractdto.LandlordPayoutType.MONTHLY);
        ...
        it('should throw error for invalid payment amount', async ()=>{
            await expect(service.processMonthlyPayment('contract-123', 0, 'card', 'ref-123')).rejects.toThrow(_common.BadRequestException);
            await expect(service.processMonthlyPayment('contract-123', -100, 'card', 'ref-123')).rejects.toThrow(_common.BadRequestException);
        });
        it('should throw error for mismatched payment amount', async ()=>{
            await expect(service.processMonthlyPayment('contract-123', 20000, 'card', 'ref-123') // Wrong amount
        ...
        it('should throw error for payment too early', async ()=>{
            ...
            await expect(service.processMonthlyPayment('contract-123', 25000, 'card', 'ref-123')).rejects.toThrow(_common.BadRequestException);
        });
        ...
        it('should handle contract not found', async ()=>{
            ...
            await expect(service.processMonthlyPayment('nonexistent', 25000, 'card', 'ref-123')).rejects.toThrow(_common.NotFoundException);
        });
    });
    ...
    describe('getLandlordEscrowBalances', ()=>{
        ...
        it('should return only unreleased escrow balances when requested', async ()=>{
            const unreleasedBalances = [
                mockEscrowBalances[0]
            ];
    ...
    describe('edge cases and error scenarios', ()=>{
        ...
        it('should validate contract expiry date boundary conditions', async ()=>{
            ...
            mockDb.where.mockReturnValue(mockDb);
            mockDb.limit.mockResolvedValue([]);
            await expect(service.createRentContract(validContractDto)).rejects.toThrow('Contract expiry date must be in the future');
        });
        it('should handle transition date calculation for edge months', async ()=>{
            ...
            const result = service.calculateTransitionStartDate(februaryDate, _tenantrentcontractdto.LandlordPayoutType.YEARLY);
            expect(result.getFullYear()).toBe(2023);
            expect(result.getMonth()).toBe(7); // August (0-indexed)
        });
        ...
        it('should handle concurrent access to same contract', async ()=>{
            ...
            await expect(service.updateNextPaymentDue('contract-123')).rejects.toThrow('Concurrent modification detected');
        });
        ...
        it('should handle very large escrow accumulations', async ()=>{
            const mockEscrowBalances = Array.from({
                length: 100
            }, (_, i)=>({
                    id: `escrow-${i}`,
                    landlordId: 'landlord-123',
        ...
        it('should handle payment processing for expired contracts', async ()=>{
            ...
            await expect(service.processMonthlyPayment('contract-123', 25000)).rejects.toThrow('Cannot process payment for expired contract');
        });
    });
});

//# sourceMappingURL=tenant-payment.service.spec.js.map