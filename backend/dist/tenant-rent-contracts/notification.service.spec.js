...
describe('NotificationService', ()=>{
    ...
    describe('schedulePaymentReminder', ()=>{
        ...
        it('should handle tenant not found', async ()=>{
            ...
            await expect(service.schedulePaymentReminder(mockContract.id, scheduledFor, 'reminder')).rejects.toThrow('Tenant not found for contract: contract-123');
        });
    ...
    describe('sendImmediateNotification', ()=>{
        ...
        it('should send immediate push notification', async ()=>{
            ...
            const result = await service.sendImmediateNotification(mockContract.tenantId, 'Payment Successful', 'Your rent payment has been processed successfully.', {
                contractId: mockContract.id
            });
        ...
        it('should handle tenant not found', async ()=>{
            ...
            mockDb.from.mockReturnValue(mockDb);
            mockDb.where.mockResolvedValue([]);
            await expect(service.sendImmediateNotification('nonexistent-tenant', 'Test', 'Test message')).rejects.toThrow('Tenant not found: nonexistent-tenant');
        });
    });
    describe('processPendingNotifications', ()=>{
        const mockPendingNotifications1 = [
            {
                id: 'notification-1',
                contractId: mockContract.id,
                tenantId: mockContract.tenantId,
        ...
        beforeEach(()=>{
            mockExpo.isExpoPushToken.mockReturnValue(true);
            mockExpo.chunkPushNotifications.mockReturnValue([
                mockPendingNotifications1.map((n)=>({
                        to: mockTenant.expoPushToken,
                        title: n.title,
                        body: n.message
                    }))
        ...
        it('should process pending notifications successfully', async ()=>{
            ...
            mockDb.orderBy.mockResolvedValue(mockPendingNotifications1);
        ...
        it('should handle notifications without push tokens', async ()=>{
            const now = new Date('2024-01-29T10:00:00.000Z');
            const notificationsWithoutTokens = mockPendingNotifications1.map((n)=>({
                    ...n,
                    tenant: {
                        ...mockTenant,
                        expoPushToken: null
    ...
    describe('updateNotificationPreferences', ()=>{
        ...
        it('should handle preference update errors', async ()=>{
            ...
            await expect(service.updateNotificationPreferences(mockTenant.id, mockPreferences)).rejects.toThrow('Update failed');
        });
    });
    describe('error handling and edge cases', ()=>{
        it('should handle Expo service unavailable', async ()=>{
            ...
            mockExpo.sendPushNotificationsAsync.mockRejectedValue(new Error('Expo service unavailable'));
            emailService.sendPaymentReminder.mockResolvedValue();
            const result = await service.sendImmediateNotification(mockContract.tenantId, 'Test', 'Test message');
            expect(result.success).toBe(true);
            expect(result.method).toBe('email');
            expect(emailService.sendPaymentReminder).toHaveBeenCalled();
        });
        ...
        it('should handle database connection errors', async ()=>{
            ...
            await expect(service.sendImmediateNotification(mockContract.tenantId, 'Test', 'Test message')).rejects.toThrow('Database connection failed');
        });
        it('should handle email service failures', async ()=>{
            ...
            emailService.sendPaymentReminder.mockRejectedValue(new Error('Email service unavailable'));
            await expect(service.sendImmediateNotification(mockContract.tenantId, 'Test', 'Test message')).rejects.toThrow('Email service unavailable');
        });
        it('should handle large batch notifications', async ()=>{
            ...
            mockExpo.sendPushNotificationsAsync.mockImplementation((chunk)=>{
                return Promise.resolve(chunk.map(()=>({
                        status: 'ok',
                        id: 'receipt'
                    })));
            });
